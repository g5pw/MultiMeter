...
- (void) parseBytes:(char *)bytes {
    ...
    number = (bytes[1] & 0x0F) * 10 ^ (range);
    number += (bytes[2] & 0x0F) * 10 ^ (range-1);
    number += (bytes[3] & 0x0F) * 10 ^ (range-2);
    number += (bytes[4] & 0x0F) * 10 ^ (range-3);
    number += (bytes[5] & 0x0F) * 10 ^ (range-4);
    // Check sign
    if ((bytes[7] & 0x04) >> 2){
        number *= -1;
    }
    NSLog(@"Got value: %f",number);
